...
const Profile = ({ isProfilePage = true }) => {
  const { profileData } = useSelector((state) => (!!isProfilePage ? state.userProfile : state.connection))
  return (
    <div className="flex-1 relative z-0 overflow-y-auto focus:outline-none xl:order-last bg-white dark:bg-card-color-dark">
      ...
      <article>
        ...
        <div>
          <div>
            <img className="h-32 w-full object-cover lg:h-48" src={profileData.coverImageUrl} alt="" />
          </div>
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="hidden sm:block 2xl:hidden mt-6 min-w-0 flex-1">
              <h1 className="text-2xl font-bold text-gray-900 truncate dark:text-title-dark">{profileData.name}</h1>
            </div>
          </div>
        </div>
